...
public class MessageController {
  ...
  public String getMessage(
      @PathVariable("id") Long distinctUserId, Model model, Principal principal) {
    final var user =
        userRepository
            .findByEmail(principal.getName())
            .orElseThrow(() -> new RuntimeException("User not found"));
    final var distinctUser =
        userRepository
            .findById(distinctUserId)
            .orElseThrow(() -> new RuntimeException("User not found"));
  ...
  public String addMessage(
      @PathVariable("id") Long distinctUserId,
    ...
    model.addAttribute("newMessage", MessageDto.builder().build());
    return "redirect:/messages/" + distinctUserId;
  }
  ...
  public String newMessage(
    ...
    final var message =
        Message.builder().sender(user).receiver(otherUser).content(messageDto.getContent()).build();
    messageRepository.save(message);
  ...
  private Optional<User> getFirstMessageUser(User user) {
    ...
    return Optional.of(
        first.getSender().getId().equals(user.getId()) ? first.getReceiver() : first.getSender());
  }
